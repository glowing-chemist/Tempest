...
int main()
{
    ...
    Camera shadowCam(float3(-0.3f, 0.1f, -0.3f), glm::normalize(float3(1.0f, -1.0f, 1.0f)), 1.0f, 0.1f, 5.0f, 90.0f, CameraMode::Orthographic);
    ...
    for(float x = -10.0f; x < 10.0f; x += 1.5f)
    {
        ...
        {
            testScene.addMeshInstance(cubeID, kInvalidInstanceID, glm::translate(float3{x, 0.5f, y}) * glm::scale(float3(30.0f, 100.0f, 30.0f)), 5, MaterialType::Albedo | MaterialType::Metalness | MaterialType::Roughness | MaterialType::Normals, "Pillar");
        }
    }
    ...
    Player* player1 = new Player(player1Instance, testScene.getMeshInstance(player1Instance), float3(0.0f, 0.0f, -0.6f), float3(0.0f, 0.0f, -1.0f));
